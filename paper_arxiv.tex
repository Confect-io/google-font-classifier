...
\begin{enumerate}[leftmargin=*]
    \item A parameter-efficient fine-tuning approach for font classification that adapts DINOv2 with LoRA, achieving 98.7\% top-1 accuracy across 394 font classes.
...
The best-performing configurations (LoRA $r{=}8$ and $r{=}16$) achieve \textbf{98.7\% top-1 accuracy} on the held-out test set across 394 font classes at 50 training epochs (see Table~\ref{tab:baselines}). Given the fine-grained nature of the task---many classes differ only in weight (e.g., Roboto-300 vs.\ Roboto-400) or subtle stylistic features---this represents strong performance.
