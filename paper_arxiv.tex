...
Training is conducted using PyTorch with HuggingFace Transformers and the PEFT library for LoRA integration. The training script automatically selects the best available hardware accelerator at runtime. Mixed-precision training (FP16) is enabled on CUDA devices to reduce memory usage and training time.
...
\begin{table}[h]
\centering
\caption{Comparison of fine-tuning strategies on DINOv2-Base with 394 font classes. All methods use the same training data, hyperparameters (where applicable), and evaluation protocol. Values will be filled after experiments are completed.}
\label{tab:baselines}
\begin{tabular}{lrrrr}
\toprule
\textbf{Method} & \textbf{Trainable Params} & \textbf{Top-1 Acc} & \textbf{Family Acc} & \textbf{SWER} \\
\midrule
ResNet-50 (CNN)      & 25.6M & --  & --  & -- \\
\midrule
