...
\begin{enumerate}[leftmargin=*]
    \item A parameter-efficient fine-tuning approach for font classification that adapts DINOv2 with LoRA, achieving 98.9\% top-1 accuracy across 394 font classes.
...
The best-performing configuration (LoRA $r{=}16$) achieves \textbf{98.9\% top-1 accuracy} on the held-out test set across 394 font classes (see Table~\ref{tab:baselines}). Given the fine-grained nature of the task---many classes differ only in weight (e.g., Roboto-300 vs.\ Roboto-400) or subtle stylistic features---this represents strong performance.
...
\begin{figure}[t]
    ...
    \caption{Top-1 variant-level accuracy plotted against the number of weight variants in each family. Families with more variants (and thus more visually similar classes to distinguish) tend to have lower accuracy, consistent with the hypothesis that intra-family weight confusion drives most classification errors.}
...
\begin{itemize}[leftmargin=*,nosep]
    ...
    \item $d \in [0.2, 0.4]$: same family, different weight---scaled linearly by the normalized weight difference (e.g., 400 vs.\ 500 yields $d \approx 0.22$; 300 vs.\ 900 yields $d = 0.4$).
...
\end{itemize}
This distance function satisfies the metric space axioms and is fully determined by font metadata---it does not depend on the model's learned representations, avoiding the circularity that would arise from using the model's own embeddings to grade its errors.
