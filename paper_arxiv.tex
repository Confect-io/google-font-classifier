...
Each dataset is publicly available on HuggingFace.\footnote{\url{https://huggingface.co/datasets/dchen0/font_crops}}\textsuperscript{,}\footnote{\url{https://huggingface.co/datasets/dchen0/font_crops_v4}}
...
The trained model is released on HuggingFace Hub as \texttt{dchen0/font\_classifier\_v4}.\footnote{\url{https://huggingface.co/dchen0/font_classifier_v4}} To simplify deployment, we merge the LoRA adapters into the base weights before upload, collapsing each low-rank update into its corresponding weight matrix (see Section~\ref{sec:lora}). The merged model produces identical outputs but no longer requires the PEFT library at inference time.
