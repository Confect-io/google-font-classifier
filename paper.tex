...
This model serves as a component of Anything,\footnote{\url{https://anything.com}} which achieved the \#1 ranking on Design Arena's builders leaderboard,\footnote{\url{https://www.designarena.ai/leaderboard/builder}} a crowdsourced benchmark for AI-generated design with over 150{,}000 voters.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{v1} (10 families, 102 variants, 91.6\% accuracy).\footnote{\url{https://huggingface.co/dchen0/font-classifier}} A small proof-of-concept trained on a handful of families (Inter, Roboto, Poppins, and several others). High accuracy, but the narrow label space did not cover enough fonts to be useful in practice.

    \item \textbf{v2} (many dozens of families, 700 variants).\footnote{\url{https://huggingface.co/dchen0/font-classifier-v2}} A broad expansion that included a large portion of the Google Fonts catalog. The much larger label space introduced many visually near-identical font pairs (e.g., families that differ only in minor spacing or hinting), causing widespread confusion between classes and degrading accuracy.

    \item \textbf{v3} (20 families, 300 variants, 59\% accuracy).\footnote{\url{https://huggingface.co/dchen0/font-classifier-v3}} Scaled back from v2 by removing duplicate-like families, but the training dataset was too small (approximately 9,400 images) relative to the label count, and several included families still overlapped visually. Accuracy dropped significantly.

    \item \textbf{v4} (31 families, 394 variants, 86\% accuracy).\footnote{\url{https://huggingface.co/dchen0/font_classifier_v4}} The current version. The font set was rebuilt from scratch using the three selection criteria described above. The training dataset was scaled up substantially (approximately 575 images per variant). A critical engineering fix---embedding preprocessing into the model's \texttt{forward} method---eliminated train-serve skew that had silently hurt earlier versions.
\end{itemize}

Each dataset is publicly available on HuggingFace (\texttt{dchen0/font\_crops} through \texttt{dchen0/font\_crops\_v4}).
...
Top-1 accuracy treats all misclassifications equally, but confusing two adjacent weights of the same family (e.g., Roboto-400 vs.\ Roboto-500) is far less severe than confusing a serif with a sans-serif. To capture this, we define a \emph{severity-weighted error rate} that uses the model's own learned representations to quantify how wrong each misclassification is.

For each of the $K = 394$ classes, we compute a centroid $\mathbf{c}_k$ by averaging the \texttt{[CLS]} token embeddings (from the final hidden layer) of all test samples belonging to class~$k$. We then compute the pairwise cosine distance between all centroids:
\begin{equation}
    d(i, j) = 1 - \frac{\mathbf{c}_i \cdot \mathbf{c}_j}{\|\mathbf{c}_i\| \, \|\mathbf{c}_j\|}
\end{equation}
and normalize by the maximum observed distance so that $d \in [0, 1]$.

Given a prediction $\hat{y}$ for a sample with true label $y$, the \emph{severity} of that prediction is:
\begin{equation}
    s(y, \hat{y}) = \begin{cases} 0 & \text{if } \hat{y} = y \\ d(y, \hat{y}) & \text{otherwise} \end{cases}
\end{equation}
The \emph{severity-weighted error rate} is then the mean severity across all $N$ test predictions:
\begin{equation}
    \text{SWER} = \frac{1}{N} \sum_{n=1}^{N} s(y_n, \hat{y}_n)
\end{equation}
This metric is 0 for a perfect classifier and increases with both the number and the perceptual severity of errors. As a calibration baseline, we compute the expected SWER under uniform random predictions: $\text{SWER}_{\text{random}} = \frac{1}{K^2} \sum_{i,j} d(i,j)$. The ratio $\text{SWER} / \text{SWER}_{\text{random}}$ measures how severe the model's errors are relative to chance.

Table~\ref{tab:severity} reports the severity-weighted metrics alongside \emph{family-level accuracy}, which collapses all weight variants into their parent family---a prediction is correct at the family level if the predicted and true labels belong to the same font family, regardless of weight. The gap between variant-level and family-level accuracy directly quantifies how much of the raw error rate is due to within-family weight confusion versus cross-family misidentification. The severity-weighted accuracy ($1 - \text{SWER}$) provides a softer, continuous view of the same phenomenon, though its absolute value depends on the embedding distance normalization. The relative severity ratio further shows that the model's errors carry only a fraction of the cost of random guessing---when the model is wrong, it errs toward visually similar classes rather than distant ones.

\begin{table}[h]
\centering
\caption{Severity-weighted error analysis. Severity is the normalized cosine distance between \texttt{[CLS]} embedding centroids of the true and predicted classes (0 = correct, 1 = maximally distant).}
\label{tab:severity}
\begin{tabular}{lr}
\toprule
...
\midrule
Misclassifications & \nMisclass{} / \nTotal{} \\
Mean misclassification severity & \meanMisclassSeverity{} \\
Severity-weighted error rate (SWER) & \swerValue{} \\
Random baseline SWER & \swerRandom{} \\
Relative severity (SWER / random) & \swerRelative{} \\
\midrule
Top-1 accuracy (variant-level) & \rawAccuracy{} \\
\textbf{Family-level accuracy} & \textbf{\familyAccuracy{}} \\
Severity-weighted accuracy ($1 - \text{SWER}$) & \severityWeightedAccuracy{} \\
\bottomrule
\end{tabular}
\end{table}
