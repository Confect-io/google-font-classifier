...
Table~\ref{tab:severity} reports the severity-weighted metrics. The model's SWER is far below the random baseline, and the relative severity ratio shows that the model's errors carry only a small fraction of the cost of random guessing. Put differently: while 14\% of predictions are incorrect by top-1 accuracy, those errors are overwhelmingly between near-identical font weights, not between typographically distant families.
