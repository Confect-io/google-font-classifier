...
Top-1 accuracy treats all misclassifications equally, but confusing two adjacent weights of the same family (e.g., Roboto-400 vs.\ Roboto-500) is far less severe than confusing a serif with a sans-serif. We define a severity-weighted error metric using the model's own learned representations: for each prediction, the severity is the normalized cosine distance between the \texttt{[CLS]} embedding centroids of the true and predicted classes (0 for correct predictions, higher for more dissimilar confusions). Figure~\ref{fig:dendrogram} shows the resulting font family hierarchy recovered by UPGMA clustering on these distances. The severity-weighted error rate is substantially lower than the rate expected under uniform random predictions, confirming that the model's errors are concentrated among visually similar classes.
