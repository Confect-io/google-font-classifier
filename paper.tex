...
Our model builds on the DINOv2-Base architecture \citep{oquab2024dinov2}, specifically the \texttt{dinov2\hbox{-}base\hbox{-}imagenet1k\hbox{-}1\hbox{-}layer} variant pre-trained on ImageNet-1K. This model uses the standard ViT-B/14 architecture with patch size 14, embedding dimension 768, 12 attention heads, and 12 transformer blocks.
...
Figure~\ref{fig:dendrogram} visualizes the font family similarity hierarchy recovered by UPGMA clustering on the centroid distances. Serif, sans-serif, monospace, and display families occupy distinct branches, confirming that the embedding space captures typographic structure. The severity-weighted error rate is substantially lower than the random baseline, confirming that when the model is wrong, it errs toward visually similar classes rather than distant ones.
